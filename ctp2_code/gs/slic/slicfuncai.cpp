...
//   These functions compile but still need to be tested. (April 22nd 2006 Martin G�hmann)
// - Provided a default tone to ParseNewProposalSlicArgs
//   if no tone is provided. (11-Oct-2007 Martin G�hmann)
// - Fixed GetPersonalityType and made it compatibe with Diplomod. (22-Oct-2007 Martin G�hmann)
//
//----------------------------------------------------------------------------
...
bool ParseNewProposalSlicArgs(SlicArgList *args, sint32 &argNum, NewProposal &new_proposal) {
	...
	sint32	tone;
	if ((static_cast<size_t>(argNum) < args->Count()) && args->GetInt(argNum++, tone))
	{
...
SFN_ERROR Slic_ConsiderStrategicState::Call(SlicArgList *args)
{
	...
	if (args->Count() > static_cast<size_t>(argNum) && args->GetInt(argNum++, spystr_id))
		state.spyStrId = spystr_id;
	if (args->Count() > static_cast<size_t>(argNum) && args->GetInt(argNum++, advicestr_id))
		state.spyStrId = advicestr_id;
	if (args->Count() > static_cast<size_t>(argNum) && args->GetInt(argNum++, newsstr_id))
...
// INT GetPersonalityType(<int|player>, <int|personaltyType>);

// returns the PersonalityDB index of player's leader's personality
// Fills the second argument if present with the personality type for mod compatibility
...
SFN_ERROR Slic_GetPersonalityType::Call(SlicArgList *args)
{
	...
	if(argNum == 2)
	{
		...
		sym->SetIntValue(m_result.m_int);
	}
